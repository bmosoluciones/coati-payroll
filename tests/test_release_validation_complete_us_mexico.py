...
from coati_payroll.model import (
    AcumuladoAnual, ConfiguracionCalculos, Deduccion, Empleado, Empresa, Moneda,
    NominaNovedad, Planilla, PlanillaDeduccion, PlanillaEmpleado, PlanillaPrestacion,
    Prestacion, PrestacionAcumulada, ReglaCalculo, TipoPlanilla, VacationAccount,
    VacationLedger, VacationNovelty, VacationPolicy, db,
)
...
def test_complete_configurable_payroll_stress_for_major_jurisdictions(app, db_session, country):
    ...
    with app.app_context():
        currency = Moneda(codigo=profile["currency"], nombre=profile["currency"], simbolo=profile["currency"], activo=True)
        company = Empresa(
            codigo=profile["company"]["code"], razon_social=profile["company"]["legal_name"],
            ruc=profile["company"]["tax_id"], primer_mes_nomina=profile["payroll"]["fiscal_start_month"],
            primer_anio_nomina=periods[0][0].year,
        )
        payroll_type = TipoPlanilla(
            codigo=profile["payroll"]["code"], descripcion=profile["name"],
            periodicidad=profile["payroll"]["frequency"], dias=profile["payroll"]["days_per_month"],
        ...
        db_session.flush()
        payroll = Planilla(
            nombre=profile["payroll"]["code"], tipo_planilla=payroll_type,
            empresa=company, moneda=currency, mes_inicio_fiscal=profile["payroll"]["fiscal_start_month"], activo=True,
        )
        config = ConfiguracionCalculos(
            empresa_id=company.id, pais_id=profile["country"],
            dias_mes_nomina=profile["payroll"]["days_per_month"], dias_anio_nomina=profile["payroll"]["days_per_year"],
            horas_jornada_diaria=Decimal(profile["payroll"]["daily_hours"]),
            dias_mes_vacaciones=profile["payroll"]["days_per_month"], dias_anio_vacaciones=profile["payroll"]["days_per_year"],
            dias_anio_financiero=profile["payroll"]["days_per_year"], meses_anio_financiero=profile["payroll"]["periods_per_year"],
            dias_quincena=profile["payroll"]["days_per_half_month"], dias_mes_antiguedad=profile["payroll"]["days_per_month"],
        ...
        employees = [
            Empleado(
                codigo_empleado=f"{country}-EMP-{index + 1:04d}", primer_nombre="Release",
                primer_apellido=f"Validation {index + 1}", identificacion_personal=f"{country}-RELEASE-{index + 1:04d}",
                fecha_alta=periods[0][0], salario_base=Decimal(stress["salary"]), moneda_id=currency.id,
                empresa_id=company.id, activo=True,
            )
        ...
        accounts = [VacationAccount(empleado_id=employee.id, policy_id=policy.id, activo=True) for employee in employees]
        db_session.add_all(accounts)

        benefits = []
        for benefit_config in stress["benefits"].values():
            benefit = Prestacion(
                codigo=benefit_config["code"], nombre=benefit_config["code"], tipo="employer",
                formula_tipo="salary_percentage", porcentaje=Decimal(benefit_config["percentage"]),
                base_calculo="salario_base", tipo_acumulacion=benefit_config["accumulation"], recurrente=True, activo=True,
            )
            benefits.append(benefit)
        db_session.add_all(benefits)
        db_session.flush()
        db_session.add_all(PlanillaPrestacion(planilla_id=payroll.id, prestacion_id=benefit.id, orden=index + 1, activo=True) for index, benefit in enumerate(benefits))

        deduction_keys = list(profile["rules"])
        for priority, rule in enumerate(profile["rules"].values(), start=1):
            deduction = Deduccion(
                codigo=rule["code"], nombre=rule["code"], formula_tipo="regla_calculo",
                es_impuesto=rule["type"] == "tax", antes_impuesto=rule["before_tax"], activo=True,
            )
            db_session.add(deduction)
            db_session.flush()
            db_session.add(ReglaCalculo(codigo=rule["code"], nombre=rule["code"], tipo_regla=rule["type"], esquema_json=rule["formula"], vigente_desde=periods[0][0], activo=True, deduccion_id=deduction.id))
        ...
        vacation_novelty = VacationNovelty(
            empleado_id=employees[usage_index].id, account_id=accounts[usage_index].id,
            start_date=usage_period[0], end_date=usage_period[0], units=Decimal(vacation_config["usage_days"]),
            estado="approved", fecha_aprobacion=usage_period[0], aprobado_por="release-validation",
        )
        db_session.add(vacation_novelty)
        db_session.flush()
        db_session.add(NominaNovedad(
            empleado_id=employees[usage_index].id, tipo_valor="dias", codigo_concepto=policy.codigo,
            valor_cantidad=Decimal(vacation_config["usage_days"]), fecha_novedad=usage_period[0],
            es_descanso_vacaciones=True, fecha_inicio_descanso=usage_period[0], fecha_fin_descanso=usage_period[0],
        ...
        for period_start, period_end in periods:
            nomina, results, errors, warnings = PayrollExecutionService(db_session).execute_payroll(
                planilla=payroll, periodo_inicio=period_start, periodo_fin=period_end,
                fecha_calculo=period_end, usuario="release-validation",
            )
            assert errors == []
        ...
        benefit_transactions = db_session.execute(db.select(PrestacionAcumulada).filter(PrestacionAcumulada.nomina_id.in_([nomina.id for nomina in executed]))).scalars().all()
        assert len(benefit_transactions) == stress["employee_count"] * len(stress["benefits"]) * len(periods)
        vacation_entries = db_session.execute(db.select(VacationLedger).join(VacationAccount).filter(VacationAccount.empleado_id.in_([employee.id for employee in employees]))).scalars().all()
        assert len(vacation_entries) == stress["employee_count"] * len(periods) + 1
        assert sum(entry.quantity for entry in vacation_entries if entry.entry_type == "usage") == -Decimal(vacation_config["usage_days"])
