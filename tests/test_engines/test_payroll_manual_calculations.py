...
MANUAL_PERCENTAGE_CASES = tuple(
    (salary, percentage, expected) for salary, row in MANUAL_PERCENTAGE_TABLE for percentage, expected in row
)
...
def test_concepts_use_expected_bases_and_round_to_cents():
    ...
    assert calculator.calculate(employee, "gross_percentage", None, Decimal("5"), None, None, None) == Decimal("825.00")

    # Hourly rate: 15,000 / 30 / 8 = 62.50; five hours at 150% = 468.75.
    assert calculator.calculate(employee, "hours", None, Decimal("150"), None, None, None, "HE") == Decimal("468.75")
...
def test_employer_benefits_do_not_reduce_employee_net():
    ...
    planilla = SimpleNamespace(
        planilla_prestaciones=[
            SimpleNamespace(activo=True, orden=1, monto_predeterminado=None, porcentaje=None, prestacion=benefit)
        ]
    )
...
def test_zero_benefit_cap_is_a_real_cap_not_an_unconfigured_cap():
    ...
    benefit = SimpleNamespace(
        id="benefit-zero-cap",
        codigo="CAP0",
        nombre="Beneficio topado a cero",
        activo=True,
        formula_tipo="fixed",
...
def test_mothers_day_bonus_requires_mother_and_mothers_day():
    mother = SimpleNamespace(
        ...
        datos_adicionales={"es_madre": True},
        salario_acumulado=0,
        impuesto_acumulado=0,
...
def test_documented_calculation_context_values_are_injected_with_executable_names():
    """The documented period and employee values are available to formulas."""
    employee = SimpleNamespace(
        fecha_alta=date(2026, 5, 10),
        fecha_nacimiento=date(1990, 5, 14),
        datos_adicionales={"region": "north"},
        salario_acumulado=0,
        impuesto_acumulado=0,
        codigo_empleado="EMP-CONTEXT",
        primer_nombre="Ada",
    )

    variables = _personal_variables(employee, date(2026, 5, 15))

    assert variables["primer_nombre"] == "Ada"
    assert variables["datos_adicionales"] == {"region": "north"}
    assert variables["salario_diario"] == Decimal("500.00")
    assert variables["salario_hora"] == Decimal("62.50")
    assert variables["edad_anios"] == Decimal("36")
    assert variables["es_nuevo_ingreso"] == Decimal("1")
    assert variables["dias_trabajados_periodo"] == Decimal("6")
    assert variables["dias_proporcional"] == Decimal("6")
    assert variables["mes_nomina"] == Decimal("5")
    assert variables["anio_nomina"] == Decimal("2026")
    assert variables["numero_periodo"] == Decimal("1")


def test_every_published_data_source_has_a_context_variable():
    """The editor cannot advertise a formula identifier that the context lacks."""
    ...
    employee = SimpleNamespace(
        ...
        salario_acumulado=0,
        impuesto_acumulado=0,
        codigo_empleado="EMP-SOURCES",
    )
    variables = _personal_variables(employee, date(2026, 5, 15))
    published = {
        get_formula_variable_name(category, field_name, field_info)
        for category, source in AVAILABLE_DATA_SOURCES.items()
        for field_name, field_info in source["fields"].items()
    }

    assert published <= variables.keys()
