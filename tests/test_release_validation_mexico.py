...
"""Mexico 2026 ISR tariff release validation through the payroll service."""
...
from coati_payroll.model import AcumuladoAnual, ConfiguracionCalculos, Deduccion, Empleado, Empresa, Moneda, Planilla, PlanillaDeduccion, PlanillaEmpleado, ReglaCalculo, TipoPlanilla, db
...
PROFILE = json.loads((Path(__file__).parents[1] / "coati_payroll/jurisdictions/mexico_2026.json").read_text(encoding="utf-8"))


def test_mexico_2026_isr_two_months_from_json_profile(app, db_session):
    case = PROFILE["cases"][0]
    start, end = date.fromisoformat(case["period_start"]), date.fromisoformat(case["period_end"])
    rule = PROFILE["rules"]["income_tax"]

    with app.app_context():
        currency = Moneda(codigo=PROFILE["currency"], nombre="Mexican Peso", simbolo="$", activo=True)
        ...
        db_session.flush()
        employee = Empleado(codigo_empleado="MX-EMP-0001", primer_nombre="Release", primer_apellido="Validation", identificacion_personal="MX-RELEASE-0001", fecha_alta=start, salario_base=Decimal(case["salary"]), moneda_id=currency.id, empresa_id=company.id, activo=True)
        db_session.add(employee)
        db_session.flush()
        db_session.add(PlanillaEmpleado(planilla_id=payroll.id, empleado_id=employee.id, activo=True))
        deduction = Deduccion(codigo=rule["code"], nombre=rule["code"], formula_tipo="regla_calculo", es_impuesto=True, antes_impuesto=False, activo=True)
        db_session.add(deduction)
        db_session.flush()
        db_session.add_all([ReglaCalculo(codigo=rule["code"], nombre=rule["code"], tipo_regla=rule["type"], esquema_json=rule["formula"], vigente_desde=start, activo=True, deduccion_id=deduction.id), PlanillaDeduccion(planilla_id=payroll.id, deduccion_id=deduction.id, prioridad=1, es_obligatoria=True)])
        db_session.commit()
        ...
        for period_start, period_end in ((start, end), (date(2026, 2, 1), date(2026, 2, 28))):
            ...
            assert employees[0].salario_bruto == Decimal(case["expected"]["gross"])
            assert employees[0].deducciones[0].monto == Decimal(case["expected"]["tax"])
            assert employees[0].salario_neto == Decimal(case["expected"]["net"])
        ...
        assert accumulated.impuesto_retenido_acumulado == Decimal("1458.04")
