...
class AccumulationProcessor:
    ...
    def update_accumulations(
        ...
        for perception_item in emp_calculo.percepciones:
            accumulated_totals[f"percepcion_{perception_item.codigo}_acumulado"] = (
                Decimal(str(accumulated_totals.get(f"percepcion_{perception_item.codigo}_acumulado", 0)))
                + perception_item.monto
            )
        for deduction_item in emp_calculo.deducciones:
            accumulated_totals[f"deduccion_{deduction_item.codigo}_acumulado"] = (
                Decimal(str(accumulated_totals.get(f"deduccion_{deduction_item.codigo}_acumulado", 0)))
