...
- Refactored payroll, loan, vacation, perception, export, and background-processing flows to reduce complexity while preserving their behavior.
- Improved payroll progress tracking, rollback helpers, report exports, and comparison validation.
- Improved accessible color contrast in the interface and hardened the requirements parser.
...
- Updated Python dependencies, including Alembic, cryptography, Flask-Caching, MySQL Connector/Python, and python-ulid.
- Refreshed the `uv.lock` lockfile for reproducible installations.
